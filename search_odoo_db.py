...
try:
    conn = psycopg2.connect(dbname='odoo', user='odoo', password='odoo', host='localhost', port=5433)
    cur = conn.cursor()
    ...
    for table in tables:
        try:
            ...
            rows = cur.fetchall()
            if rows:
                print(f"Found in odoo table '{table}':")
                for row in rows:
                    print(row[:5])
                found = True
    ...
    if not found:
        print("Not found in database odoo.")
        
    cur.close()
    conn.close()
except Exception as e:
    print("Could not connect to database odoo:", e)
